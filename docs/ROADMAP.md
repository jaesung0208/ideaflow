...
| 전체 진행률 | 25% |
| 현재 Phase | Phase 2 (예정) |
| 다음 마일스톤 | Phase 2 완료 - 링크 공유 실시간 동기화 MVP |
...
| AI | Gemini 2.5 Flash | Google AI Studio 무료 티어, 클러스터링에 충분한 성능 |
...
- 캔버스 UI (Phase 1) -> 드래그/핀치줌 제스처 (Phase 3)는 기본 UI 위에 구현
...
- karpathy-guidelines 준수: 이 단계에서는 Firebase 코드를 미리 작성하지 않음, 로컬 상태만으로 동작하는 최소 구현

---

## Phase 2: Firebase 연동 및 실시간 동기화 (Sprint 2) 📋
...
### 기술 고려사항
- Firebase 설정은 환경변수(`.env.local`)로 관리, `.env.local.example` 파일 제공
- Firestore 보안 규칙은 최소 권한 원칙 적용 (해당 방의 문서만 접근 가능)
- Optimistic UI 구현 시 `onSnapshot`의 `metadata.hasPendingWrites`를 활용하여 로컬/서버 상태 구분
- throttling은 `lodash/throttle` 또는 직접 구현 (의존성 최소화 원칙에 따라 직접 구현 권장)
- karpathy-guidelines 준수: Firebase 연동 코드를 훅 단위로 격리, 컴포넌트가 Firebase에 직접 의존하지 않도록 설계

---

## Phase 3: 모바일 최적화 및 음성 메모 (Sprint 3) 📋
...
### 완료 기준 (Definition of Done)
- 모바일 디바이스(또는 Chrome DevTools 모바일 에뮬레이션)에서 핀치줌으로 캔버스 확대/축소 동작
- 포스트잇 드래그가 터치 환경에서 부드럽게 동작 (60fps 유지)
- 음성 메모 버튼 클릭 시 마이크 권한 요청 후 음성 인식이 시작됨
- 인식된 음성이 새 포스트잇으로 생성됨
- Web Speech API 미지원 브라우저에서 마이크 버튼이 비활성화됨
- 모바일 뷰포트(375px)에서 모든 버튼의 터치 타겟이 48px 이상
...
2. `browser_snapshot` -> 데스크톱 레이아웃 확인 (넓은 캔버스)
...
- karpathy-guidelines 준수: 핀치줌은 충분히 복잡하므로 별도 훅(`useCanvasGesture`)으로 격리, 캔버스 컴포넌트에는 최소 인터페이스만 노출
...
### 목표
Gemini 2.5 Flash를 활용하여 흩어진 아이디어를 자동으로 그룹화하는 스마트 클러스터링 기능을 구현하고, Vercel에 최종 배포하여 프로덕션 준비를 완료한다.

### 작업 목록 (Must Have)

- ⬜ **Gemini AI 연동** (복잡도: 중간)
  - Next.js API Route (Route Handler)로 Gemini API 프록시 엔드포인트 생성
  - API 키는 서버 측 환경변수로 관리 (클라이언트에 노출하지 않음)
  - 요청 rate limiting: 사용자당 분당 5회 제한 (무료 티어 RPM 보호)

- ⬜ **스마트 클러스터링 기능** (복잡도: 높음)
  - 현재 방의 모든 노트 텍스트를 수집하여 Gemini에 클러스터링 요청
  - 프롬프트 설계: "다음 아이디어들을 유사한 주제별로 그룹화하고 그룹명을 제안하세요"
...
- ⬜ **Vercel 배포** (복잡도: 낮음)
  - Vercel 프로젝트 연결 및 환경변수 설정 (Firebase 키, Gemini API 키)
...
- ⬜ **성능 최적화** (복잡도: 중간)
  - 노트 100개 이상일 때 가상화(Virtualization) 또는 뷰포트 내 노트만 렌더링
  - Firestore 쿼리 최적화: 방별 인덱스 활용
  - Next.js Image 컴포넌트 불필요 (텍스트 기반), 번들 사이즈 최소화
  - Lighthouse 성능 점수 90 이상 달성

- ⬜ **에러 처리 및 안정성** (복잡도: 낮음)
  - Firebase 연결 실패 시 재연결 로직 및 사용자 알림
  - Gemini API 실패 시 에러 메시지 표시 ("AI 기능을 일시적으로 사용할 수 없습니다")
...
9. `browser_network_requests` -> Gemini API 프록시 호출 성공(200) 확인
...
5. `browser_network_requests` -> Firebase/Gemini 연동 정상 확인
6. `browser_console_messages(level: "error")` -> 프로덕션 에러 없음 확인

### 기술 고려사항
- Gemini API 호출은 반드시 서버 측(Route Handler)에서 수행하여 API 키 보호
- 클러스터링 프롬프트는 구조화된 JSON 응답을 요구하여 파싱 안정성 확보
- 노트 재배치 애니메이션은 Framer Motion `animate`로 부드럽게 처리 (layout animation)
- rate limiting은 메모리 기반으로 간단히 구현 (서버리스 환경에서는 요청별 독립이므로 클라이언트 측에서도 제한)
- karpathy-guidelines 준수: AI 프롬프트와 파싱 로직을 별도 유틸리티로 분리, 컴포넌트에서는 "클러스터링 결과"만 받아 사용
...
|--------|--------|-------------|-----------|
| Firebase 무료 할당량 초과 | 높음 | 중간 | 커서 throttling(100ms), 불필요한 쓰기 최소화, 사용량 모니터링 대시보드 확인 |
| Web Speech API 브라우저 호환성 | 중간 | 높음 | 미지원 시 기능 비활성화(graceful degradation), 사용자에게 지원 브라우저 안내 |
| Gemini 무료 티어 RPM 제한 | 중간 | 중간 | 사용자당 rate limiting, 클러스터링 결과 캐싱, 불필요한 재요청 방지 |
| 모바일 터치 제스처 복잡성 | 중간 | 높음 | Phase 1에서 기본 드래그 검증 후 Phase 3에서 점진적으로 제스처 추가 |
| 실시간 동기화 충돌 | 높음 | 낮음 | editorId 기반 잠금, Firestore 트랜잭션 활용, 최후의 쓰기 우선(last-write-wins) 정책 |
...
|----------|-------|-------------|--------|
| M1: 정적 프로토타입 | Phase 1 완료 | 2026-03-27 | 로컬 동작 캔버스 데모 |
...
| 항목 | 발생 시점 | 해소 시점 | 설명 |
|------|-----------|-----------|------|
| 로컬 상태 -> Firestore 전환 | Phase 1 | Phase 2 | Phase 1의 로컬 상태 코드를 Firestore로 교체 |
