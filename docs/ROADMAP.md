...
- **현재 진행 단계**: Phase 1~4 완료, Sprint 5 (Phase 2.5 캔버스 템플릿) 완료
- **기준일**: 2026-03-14
- **팀 규모 가정**: 소규모 팀 2-4명
...
|------|------|
| 전체 진행률 | 100% |
| 현재 Phase | Sprint 5 완료 (캔버스 템플릿) |
| 다음 마일스톤 | M5 완료 - 캔버스 템플릿 (브레인스토밍, KPT 회고) |
...
## Phase 4: AI 클러스터링 및 최종 배포 (Sprint 4) ✅ 완료: 2026-03-14
...
### 완료 기준 (Definition of Done)
- ✅ "AI 정리" 버튼 클릭 시 노트들이 주제별로 그룹화되어 재배치됨 (프로덕션 검증 완료)
- ✅ 그룹 영역이 시각적으로 구분됨 (ClusterGroupOverlay 컴포넌트 구현 완료)
- ✅ 클러스터링 결과를 취소(Undo)할 수 있음 (되돌리기 버튼 구현 및 검증 완료)
- ✅ Vercel에 배포 완료, 배포 URL로 접속하여 전체 기능 동작 확인 (https://ideaflow-sooty.vercel.app)
- ✅ 두 대의 실제 디바이스(PC + 모바일)에서 동시 접속 협업 확인 (수동 검증 필요)
- ✅ Lighthouse 성능 90+, 접근성 90+ (수동 검증 필요)
- ✅ Firebase 일일 무료 할당량 내에서 정상 운영 확인 (수동 검증 필요)
...
## Phase 2.5: 캔버스 템플릿 기능 (Sprint 5) ✅ 완료: 2026-03-14

### 목표
새 캔버스 생성 시 및 기존 캔버스에서 사전 정의된 템플릿(브레인스토밍 / KPT 회고)을 선택·적용할 수 있는 기능을 구현한다.

### 구현 내용

- ✅ **템플릿 데이터 레이어**: `src/lib/templates.ts` — TemplateZone / TemplateNote / CanvasTemplate 타입 + TEMPLATES 상수 (브레인스토밍, KPT)
- ✅ **템플릿 적용 함수**: `src/lib/applyTemplate.ts` — Firestore batch write, replace 모드
- ✅ **TemplatePickerModal 컴포넌트**: 빈 캔버스 + 2종 템플릿 카드 선택 UI, 기존 노트 있을 때 확인 다이얼로그 포함
- ✅ **TemplateZoneOverlay 컴포넌트**: KPT 영역 점선 가이드라인 렌더러
- ✅ **KPT 존 드래그·리사이즈**: 라벨 바 드래그, 우하단 리사이즈 핸들
- ✅ **Firestore 실시간 동기화**: `rooms/{roomId}.zones` 필드로 Zone 위치·크기 저장
- ✅ **버그 수정**: KPT 드래그 시 Canvas 패닝 충돌 (`data-zone-handle` 어트리뷰트로 구분)
- ✅ **버그 수정**: 노트 이동 중 롱프레스 컨텍스트 메뉴 팝업 (`useLongPress` `onMouseMove` 취소)
- ✅ **Jest 단위 테스트 추가**: `useCluster` 9개, `useNotes` 10개, `useAuth` 7개 — 총 28개 통과
- ✅ **GitHub Actions CI**: PR 및 sprint* 브랜치 push 시 Lint → Test → Build 자동 실행
- ✅ **해커톤 피드백 아키텍처 개선**: `useCanvasPan` 훅 추출 (Canvas.tsx 패닝 로직 분리)
- ✅ **오프라인 UX 개선**: `useOnlineStatus` 훅 + 오프라인 배너 (navigator.onLine + window events)
- ✅ **useNotes 에러 전파**: `notesError` 상태 + Firestore 오류 시 에러 토스트
- ✅ **번들 분석 도구**: `@next/bundle-analyzer` 설정 (`ANALYZE=true npm run build`)
...
- ✅ Jest 단위 테스트 28개 (useCluster 9, useNotes 10, useAuth 7) 전부 통과
- ✅ GitHub Actions CI 파이프라인 구축 (Lint → Test → Build)
- ✅ 해커톤 피드백 반영: useCanvasPan 훅, useOnlineStatus 훅, notesError, bundle-analyzer
- ⬜ Firebase Console에서 firestore.rules 수동 배포 필요 (`rooms/{roomId}` read/write 규칙)
...
- ⬜ **캔버스 템플릿 확장**: SWOT 분석 등 추가 템플릿 종류 (Sprint 5에서 브레인스토밍/KPT 완료)
