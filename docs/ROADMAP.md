...
- ⬜ 각 탭에 서로 다른 익명 사용자 ID/색상이 부여됨 (수동 검증 필요)
- ⬜ 커서 위치가 다른 탭에서 실시간으로 표시됨 (수동 검증 필요)
- ⬜ Firebase Firestore 쓰기가 일일 무료 할당량(2만) 내에서 합리적으로 소비됨 (수동 검증 필요)
- ✅ `npm run build` 에러 없음
...
- ⬜ **Gemini AI 연동** (복잡도: 중간, 예상: 4시간)
  - Next.js Route Handler(`app/api/cluster/route.ts`)로 Gemini API 프록시 엔드포인트 생성
...
- karpathy-guidelines 준수:
  - AI 프롬프트와 파싱 로직을 별도 유틸리티(`lib/gemini.ts`)로 분리
